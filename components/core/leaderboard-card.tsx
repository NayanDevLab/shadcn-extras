...
const toneMap: Record<Tone, {
  card: string; pill: string; progressTrack: string; progressFill: string; name: string
}> = {
  emerald: {
...
const sizeMap: Record<Size, { pad: string; name: string; pill: string; caption: string; avatar: string }> = {
  sm: { pad: 'p-4', name: 'text-base', pill: 'text-[11px] px-2.5 py-1', caption: 'text-xs', avatar: 'h-14 w-14' },
  md: { pad: 'p-6', name: 'text-lg',   pill: 'text-xs px-3 py-1.5',      caption: 'text-sm', avatar: 'h-16 w-16' },
  lg: { pad: 'p-8', name: 'text-xl',   pill: 'text-sm px-3.5 py-1.5',    caption: 'text-sm', avatar: 'h-20 w-20' },
}
...
export function LeaderboardCard({
  ...
  return (
    <div className={cn('relative w-full rounded-xl shadow-sm', t.card, s.pad, className)}>
      {/* avatar */}
      <div className="flex w-full flex-col items-center gap-3">
          ...
          <div className={cn(
            ...
            {avatarSrc ? (
              ...
              <div className="flex h-full w-full items-center justify-center rounded-full bg-zinc-200 dark:bg-zinc-800">
                <span className="text-zinc-500 dark:text-zinc-400">{name?.[0] ?? '?'}</span>
              </div>
            )}
            {showCrown && (
              <Crown className="absolute -top-4 left-1/2 -translate-x-1/2 h-5 w-5 text-yellow-400 drop-shadow" />
            )}
          </div>
          {typeof rank === 'number' && (
        ...
        {/* name + amount pill */}
        <div className={cn('text-center font-semibold', t.name, s.name)}>{name}</div>
        {amountLabel && (
          <span className={cn('rounded-full font-semibold', t.pill, s.pill, pillClassName)}>{amountLabel}</span>
        )}
      </div>
      ...
      <div className="mt-6">
        ...
        <div className={cn('h-2 w-full overflow-hidden rounded-full', t.progressTrack)}>
          <div
            className={cn('h-full rounded-full', progressClassName ?? t.progressFill)}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
